...
export async function getById(id: string) {
  const result = await prisma.eCClaim.findUnique({ where: { id }, include: { student: { include: { person: true } }, moduleRegistration: { include: { module: true } } } });
...
export async function remove(id: string, userId: string, req: Request) {
  const previous = await getById(id);
  await prisma.eCClaim.delete({ where: { id } });
