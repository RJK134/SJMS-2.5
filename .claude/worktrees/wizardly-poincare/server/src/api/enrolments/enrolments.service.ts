import prisma from '../../utils/prisma';
import { logAudit } from '../../utils/audit';
import { emitEvent } from '../../utils/webhooks';
import { NotFoundError } from '../../utils/errors';
import { buildPaginatedResponse } from '../../utils/pagination';
import type { Request } from 'express';

export async function list(query: Record<string, any>) {
  const { page, limit, sort, order, search, ...filters } = query;
  const skip = (page - 1) * limit;
  const where: Record<string, any> = {
    deletedAt: null,
    
    ...(filters.studentId ? { studentId: filters.studentId as any } : {}),
      ...(filters.academicYear ? { academicYear: filters.academicYear as any } : {}),
  };
  const [data, total] = await Promise.all([
    prisma.enrolment.findMany({ where, skip, take: limit, orderBy: { [sort]: order } as any }),
    prisma.enrolment.count({ where }),
  ]);
  return buildPaginatedResponse(data, total, { page, limit, skip, sort, order });
}

export async function getById(id: string) {
  const result = await prisma.enrolment.findUnique({ where: { id }, include: { student: { include: { person: true } }, programme: true, moduleRegistrations: { where: { deletedAt: null }, include: { module: true } } } });
  if (!result) throw new NotFoundError('Enrolment', id);
  return result;
}

export async function create(data: any, userId: string, req: Request) {
  const result = await prisma.enrolment.create({ data });
  await logAudit('Enrolment', result.id, 'CREATE', userId, null, result, req);
  await emitEvent('enrolments.created', { id: result.id });
  return result;
}

export async function update(id: string, data: any, userId: string, req: Request) {
  const previous = await getById(id);
  const result = await prisma.enrolment.update({ where: { id }, data });
  await logAudit('Enrolment', id, 'UPDATE', userId, previous, result, req);
  await emitEvent('enrolments.updated', { id });
  return result;
}

export async function remove(id: string, userId: string, req: Request) {
  const previous = await getById(id);
  await prisma.enrolment.update({ where: { id }, data: { deletedAt: new Date() } });
  await logAudit('Enrolment', id, 'DELETE', userId, previous, null, req);
  await emitEvent('enrolments.deleted', { id });
}
