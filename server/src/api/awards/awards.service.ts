...
export async function getById(id: string) {
  const result = await prisma.awardRecord.findFirst({ where: { id, deletedAt: null }, include: { student: { include: { person: true } }, programme: true, enrolment: true, degreeCalculation: true } });
...
export async function remove(id: string, userId: string, req: Request) {
  const previous = await getById(id);
  await prisma.awardRecord.update({ where: { id }, data: { deletedAt: new Date() } });
