...
export async function getById(id: string) {
  const result = await prisma.applicationReference.findFirst({ where: { id, deletedAt: null }, include: { application: true } });
...
export async function remove(id: string, userId: string, req: Request) {
  const previous = await getById(id);
  await prisma.applicationReference.update({ where: { id }, data: { deletedAt: new Date() } });
