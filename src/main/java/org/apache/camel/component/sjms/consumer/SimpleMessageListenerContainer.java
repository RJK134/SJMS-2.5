...
public class SimpleMessageListenerContainer extends ServiceSupport
    ...
    private final AtomicBoolean recoveringInProgress = new AtomicBoolean(false);
    ...
    protected void recoverConnection() {
        if (!isRunAllowed()) {
            recoveringInProgress.set(false);
            return;
        }
        LOG.debug("Recovering from JMS Connection exception");
        try {
            refreshConnection();
            initConsumers();
            LOG.debug("Successfully recovered JMS Connection");
            recoveringInProgress.set(false);
        } catch (Exception e) {
            LOG.warn("Failed to recover JMS Connection. Will try again in {} millis", endpoint.getRecoveryInterval(), e);
            // Schedule another attempt
            if (recoverPool != null && isRunAllowed()) {
                recoverPool.schedule(this::recoverConnection, endpoint.getRecoveryInterval(), TimeUnit.MILLISECONDS);
            } else {
                recoveringInProgress.set(false);
            }
        }
    ...
    protected void scheduleConnectionRecovery() {
        ...
        try {
            ...
            if (recoveringInProgress.compareAndSet(false, true)) {
                recoverFuture = recoverPool.schedule(this::recoverConnection, endpoint.getRecoveryInterval(), TimeUnit.MILLISECONDS);
            }
        } finally {
            connectionLock.unlock();
        }
