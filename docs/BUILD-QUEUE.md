...
- 16C **Applicant-to-student conversion and enrolment orchestration — IN FLIGHT** (exported `convertApplicantToStudentOnFirm(applicationId, userId, req)` on `applications.service`; idempotent lookup of Student by `personId` and Enrolment by `{studentId, programmeId, academicYear, deletedAt: null}` via new repository helpers; delegates creation to `students.service.create` / `enrolments.service.create` so existing audit + `students.created` / `enrolment.created` events fire naturally; dedicated `application.firm_accepted` event emitted only when at least one of Student or Enrolment was newly created; wired into `applications.service.update()` via a `safeConvertApplicantToStudentOnFirm` fail-soft wrapper that runs only on transitions INTO `FIRM` and never propagates failure to the HTTP caller; new `server/src/utils/student-number.ts` generator; 10 new Vitest cases — 7 direct on the converter and 3 on the `update()` integration; KI-P16C-001 logged for the `HOME` fee-status default placeholder; KI-P16C-002 logged for the non-transactional nature of the Student + Enrolment pair)
