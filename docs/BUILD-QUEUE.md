...
| Check | Result | Notes |
|---|---|---|
| `cd server && npx tsc --noEmit` | 🟠 pre-existing | 1 error — TS5101 on `baseUrl` deprecation introduced by the TypeScript 6.0 dependabot bump (PR #69). Not caused by Phase 16. Logged as **KI-P16-001**. Phase 16A–16D introduce 0 new tsc errors. |
...
| Verification protocol Gates 1–12 | 🟠 mixed | Gates 2–12 green. Gate 1 (server tsc clean) is red on `main` today because of KI-P16-001; Batches 16A–16D ship non-regression rather than a new fix. |
...
- 16E Applicant/admin portal completion for this journey — including a "Convert to Student" affordance on `client/src/pages/admissions/ApplicationDetail.tsx` calling the existing `POST /applications/:id/convert` endpoint
